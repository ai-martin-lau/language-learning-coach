...
**第一轮就进入一个真实旅行任务，在实际表现支持的范围内，沿最短实用路径建立可验证的 A2 基础。**
...
教练会缩短从“想学”到实际买票、入住、点餐、问路或修复沟通的距离。第一课选择下一次旅行最可能发生的一个任务，只教完成它所需的一至三个完整语块，并立即让你使用。

它不会跳过可靠输入、变化条件练习或延迟复测。这里的“快速”，是减少与近期旅行无关的词表、语法顺序、通用学习日历和材料，而不是承诺立刻达到 A2、固定完成时间或降低掌握标准。

## 为旅行而学，不为打卡而学

![六个核心旅行训练场景：交通、住宿、餐饮、问路、购物和沟通修复](assets/readme/travel-scenarios.svg)

默认旅行路线跟踪七类结果。图中展示六个日常互动核心域，第七个是有安全边界的基础求助：
...
- **基础求助：** 处理药店、遗失物或其他非紧急求助，不把严重突发情况包装成 A2 任务。
...
工作区会记录任务、提示级别、证据环境、**回答媒介**、结果、日期和下次复测。这样，罗马字输入、不可观察的“我说了”或仍在等待作答的任务，都不会被误存成可听口语或目标文字写作。旅行任务只有在存在匹配证据时，才会依次推进为同课完成、变化迁移、延迟保持和现实验证。

完成一个任务不等于达到 A2。内部 A2 风格筛查只有在模板七个旅行任务域中至少五个以不同核心语块完成延迟或现实验证、包含沟通修复、六个能力维度都有任务地图精确引用的延迟保持证据，并至少有一个真人或现实任务通过时才算通过。此后教练也只能说“证据与已测试任务中的 A2 风格表现一致”，并明确列出缺口；这仍不是 CEFR 结果，正式等级只能由合适的外部测评确认。进步速度取决于语言距离、起点、练习时间、资料质量，以及表现能否经得住迁移和延迟。
...
├── phrase-bank.md
├── function-map.md
└── progress.md       # 旅行任务、声音—文字基础支线、复测和课次证据
```
...
验证缺少声音—文字表的旧工作区前，内置增量迁移器只插入一张当前空表，并保留原有课次、证据和备注；它不会反推过去学过的基础内容。
