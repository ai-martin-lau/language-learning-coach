# 学习进展

## 下次学习

- 到期项目：待填写
- 本次唯一重点：待填写
- 最小完成任务：待填写
- 建议时长：待填写

## 旅行任务地图

<!--
只启用与用户目标有关的任务。状态依次为 planned／training／same_session_passed／changed_condition_passed／delayed_passed／field_checked；不选写 not_selected。
胜利条件是给学习者阅读的任务说明；证据要求是状态推进的机器权威。两者必须由教练保持语义一致，但验证器不会猜测自然语言含义。
证据要求逐项写成 `P001:dimension:role`，多项用逗号分隔；role 只用 core／follow_up／repair。通过态至少包含 core 和 follow_up／repair 之一，并要求每一项证据都达到对应门槛。
单课成功不是 A2。changed_condition_passed 需要变化迁移，delayed_passed 需要延迟无答案复测，field_checked 需要无答案的真人或现实互动通过。
-->

| 任务编号 | 任务域 | 胜利条件 | 证据要求 | 状态 | 最近证据 | 下一变化 |
|---|---|---|---|---|---|---|
| M01 | transport | 待用户选择具体任务 | — | `not_selected` | — | — |
| M02 | lodging | 待用户选择具体任务 | — | `not_selected` | — | — |
| M03 | eating | 待用户选择具体任务 | — | `not_selected` | — | — |
| M04 | shopping | 待用户选择具体任务 | — | `not_selected` | — | — |
| M05 | directions_local_geography | 待用户选择具体任务 | — | `not_selected` | — | — |
| M06 | communication_repair | 待用户选择具体任务 | — | `not_selected` | — | — |
| M07 | basic_help | 待用户选择具体任务 | — | `not_selected` | — | — |

## A2 风格内部筛查

<!--
这不是 CEFR 证书。状态只用 not_ready／evidence_consistent_in_tested_tasks。not_ready 时前三个摘要格保留 `—`，结论固定为模板中的保守文案。
后者要求至少五个不同旅行任务域达到 delayed_passed 或 field_checked，包含 communication_repair，六个能力维度都有任务地图精确引用的 retained 证据，并至少有一个 field_checked。验证器通过前不得输出 A2-style 结论。
通过时，达标任务域用 canonical 域名逗号分隔，能力覆盖用六个维度枚举逗号分隔，现实检查填写实际 `field_checked` 的任务编号；结论固定写 `证据与已测试旅行任务中的 A2 风格表现一致；正式 CEFR 未确认`。不得手写与任务地图不一致或更强的结论。
-->

| 筛查编号 | 状态 | 达标任务域 | 能力覆盖 | 现实检查 | 结论 |
|---|---|---|---|---|---|
| A2S01 | `not_ready` | — | — | — | 只报告单项任务的实际证据阶梯 |

## 复测队列

<!-- 提示级别沿用 phrase-bank.md；任务中不写出答案。 -->

| 编号 | 维度 | 到期日 | 无答案任务 | 提示级别 | 迁移条件 | 安排原因 |
|---|---|---|---|---|---|---|
|  |  |  |  |  |  |  |

## 反复错误队列

<!--
一次错误只记 observing 假设，不给学习者贴标签。只有同一模式累计在至少两个不同真实课次日期被观察到，才改为 recurring。
类别只用 meaning／form_retrieval／register／script／interaction_repair／pronunciation／other；状态只用 observing／recurring／resolved。
观察日期和关联语块可用逗号分隔；日期必须对应课次标题，语块必须存在。每项都安排一个不含完整答案、需要学习者做出区分或提取的下一辨别任务。观察必须来自实际可观察媒介，文字或自报不能生成发音错误结论。
resolved 只表示无答案变化任务成功后不再优先，不表示永久消失或整体掌握，也不升级其他证据。
-->

| 模式编号 | 类别 | 观察日期 | 关联语块 | 观察到的问题 | 下一辨别任务 | 状态 |
|---|---|---|---|---|---|---|
|  |  |  |  |  |  |  |

## 生活嵌入与真实使用

- 独白／情境预演／短日记：待填写
- 兴趣输入：待填写
- 真人或现实任务检查点：待填写
- 当前检查点证据：待填写（AI 角色扮演只记为练习）
- 微沉浸状态：off
- 微沉浸触发与单项动作：—

## 课次记录

<!-- 每次学习在顶部追加一个区块；只写本课产生的可观察证据。 -->

### YYYY-MM-DD

- 模式：极简／标准／深度／维护／实战复盘
- 已完成任务：待填写
- 新语块：待填写
- 示范来源与交付：待填写
- 分技能证据：待填写（语块编号、维度、提示级别、结果、证据环境、回答媒介）
- 本次只纠正的关键问题：待填写
- 重试或迁移结果：待填写
- 习惯执行：待填写（触发点、最低任务或备用动作）
- 生活嵌入动作：待填写
- 真实使用证据：待填写（区分 AI 练习与真人／现实任务）
- 下次复测：待填写
- 下一步：待填写
